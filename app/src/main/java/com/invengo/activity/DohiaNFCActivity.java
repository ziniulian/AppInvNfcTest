...
public class DohiaNFCActivity extends ListActivity {
	...
	private static final String LOG_TAG = DohiaNFCActivity.class.getSimpleName();
	private static final String DOHIA_PREFIX = "16580";
	...
	protected void onListItemClick(ListView listView, View view, int position, long id) {
		TagEntity entity = (TagEntity) listView.getAdapter().getItem(position);
		String tagId = entity.getTagId();
		if(!tagId.startsWith(DOHIA_PREFIX)){
			Toast.makeText(this, R.string.toast_dohia_failure, Toast.LENGTH_LONG).show();
		}else{
			String endId = tagId.substring(tagId.length() - 2);
			int endInt = Integer.parseInt(endId, 16);
			Intent dohiaIntent = new Intent(this, DohiaResultActivity.class);
			if((endInt % 2) == 0){
				Log.i(LOG_TAG, "INFO.Dohia vip.");
				int resourceId = R.drawable.dohia_vip;
				dohiaIntent.putExtra("DOHIA", resourceId);
			}else {
				Log.i(LOG_TAG, "INFO.Dohia red.");
				int resourceId = R.drawable.dohia_red;
				dohiaIntent.putExtra("DOHIA", resourceId);
			}
			startActivity(dohiaIntent);
		}
	}
